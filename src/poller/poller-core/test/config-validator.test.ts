        ...
        .should.be.rejectedWith(
          new ValidationError(
            'Invalid Autoscaler Configuration parameters:\n' +
              'MemorystoreConfig must be array',
          ),
        );
    });
    ...
    it('fails when given an empty array', async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            'Invalid Autoscaler Configuration parameters:\n' +
              'MemorystoreConfig must NOT have fewer than 1 items',
          ),
        );
    });
        ...
        .should.be.rejectedWith(
          new ValidationError(
            'Invalid Autoscaler Configuration parameters:\n' +
              ...
              "MemorystoreConfig/0 must have required property 'clusterId'",
          ),
        );
    });
    ...
    it('fails with an invalid property ', async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            'Invalid Autoscaler Configuration parameters:\n' +
              'MemorystoreConfig/0 must NOT have additional properties',
          ),
        );
    });
    ...
    it('fails when a property is not valid', async () => {
      await configValidator
        ...
        .should.be.rejectedWith(
          new ValidationError(
            'Invalid Autoscaler Configuration parameters:\n' +
              'MemorystoreConfig/0/minSize must be number',
          ),
        );
    });
